...
public class LoginFragment extends Fragment implements Validator.ValidationListener {
    ...
    public void exportDb()
    {
        try {
            ...
            if (sd.canWrite()) {
                String currentDBPath = "data/br.com.alois.aloismobile/databases/alois.db";
                String backupDBPath = "/alois.db"; //"{database name}";
                File dir = new File(sd,backupDBPath.replace("alois.db",""));
                if(dir.mkdir()) {
            ...
            Toast.makeText(this.getContext(), "Exportou o db", Toast.LENGTH_SHORT).show();
        } catch (Exception e) {
